...
export default function TaskPage(props: TaskPageProps) {
    ...
    const { data: tasks = [], isLoading, error } = useTasksQuery(props.workspaceId);
    const createTaskMutation = useCreateTaskMutation(props.workspaceId);
    const updateTaskMutation = useUpdateTaskMutation(props.workspaceId);
    const deleteTaskMutation = useDeleteTaskMutation(props.workspaceId);

    const handleCreateTask = async (task: TaskCreateDto) => {
        createTaskMutation.mutate(task, {
            onSuccess: () => setMessageLocal(`Task "${task.title}" has been created successfully!`),
            onError: (error: unknown) => setMessageLocal(error instanceof Error ? error.message : "Failed to create task."),
        });
    };
    ...
    const handleDeleteTask = async (taskId: number) => {
        deleteTaskMutation.mutate(taskId, {
            ...
            onError: (error: unknown) => setMessageLocal(error instanceof Error ? error.message : "Failed to delete task."),
        });
    };

    const handleUpdateTask = async (updatedTask: TaskUpdateDto, taskId: number) => {
        updateTaskMutation.mutate({ updatedTask, taskId }, {
            onSuccess: () => {
                setEditingTask(null);
                setMessageLocal(`Task "${updatedTask.title}" has been updated successfully!`);
            },
            onError: (error: unknown) => setMessageLocal(error instanceof Error ? error.message : "Failed to update task."),
        });
    };
    ...
    return (
        <div className="w-full max-w-2xl bg-white rounded-xl shadow-lg p-8">
            ...
            <div className="mt-8">
                ...
                <TaskCreationForm workspaceId={props.workspaceId} onSubmit={handleCreateTask} />
            </div>
        </div>
    );
}